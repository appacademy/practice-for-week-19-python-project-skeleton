...
const CreateRestaurant = () => {
  ...
  const handleNewRestaurant = async (e) => {
    ...
    if (urls.length > 0) {
      if (!urls[0].match(/\.(png|jpe?g)$/) || !urls[0]) {
        errors.urls = "Image URL must end in .png, .jpg, or .jpeg!";
      }
    }
    setErrors(errors);
        ...
        if (createdRestaurant) {
          urls?.forEach(async (url) => {
            if (url) {
              let payload = { url: url };
              await dispatch(
                createRestaurantImage(createdRestaurant.id, payload)
              );
            }
          });
        }
  ...
  return (
    <section className="create-restaurant-container">
      ...
      <form onSubmit={handleNewRestaurant} className="create-restaurant-form">
        ...
        <div className="form-group">
          <select onChange={updatePrice} className="input-field" required>
